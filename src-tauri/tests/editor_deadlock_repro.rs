...
use kode_lib::commands::editor::{open_buffer, get_highlights, edit_buffer, close_buffer};
...
async fn test_read_file_then_open_buffer() {
    ...
    assert!(result.is_ok(), "open_buffer HUNG - DEADLOCK! Should complete within 2 seconds");
...
async fn test_open_buffer_while_parsing() {
    ...
    let highlights_task = tokio::spawn({
        ...
        async move {
            get_highlights(path, 0, 10).await
        }
    });
    ...
    assert!(result.is_ok(), "File B's open_buffer HUNG while file A was parsing - DEADLOCK!");
...
async fn test_open_buffer_while_editing() {
    ...
    assert!(result.is_ok(), "File B's open_buffer HUNG while file A was being edited - DEADLOCK!");
...
async fn test_concurrent_highlights_and_open() {
    ...
    for i in 0..3 {
        ...
        highlight_tasks.push(tokio::spawn(async move {
            get_highlights(path, 0, 50).await
        }));
    }
    ...
    assert!(result.is_ok(), "4th file's open_buffer HUNG while 3 files were parsing - DEADLOCK!");
...
async fn test_full_frontend_sequence() {
    ...
    for (highlights1, highlights2) in results {
        assert_eq!(highlights1.lines.len(), 0, "First highlights should be empty");
        assert!(highlights2.lines.len() > 0 || highlights2.total_lines > 0, "Second highlights should have data");
    }
...
async fn test_stress_ten_files_concurrent() {
    let files: Vec<_> = (0..10)
        .map(|i| create_temp_js_file(&format!(
